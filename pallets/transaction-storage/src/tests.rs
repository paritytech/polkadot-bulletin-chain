...
use super::{
	...
	TransactionKind, AUTHORIZATION_NOT_EXPIRED, BAD_DATA_SIZE, CHAIN_PERMANENT_CAP_REACHED,
...
fn authorize_account_after_expiry_resets_bytes_permanent() {
	new_test_ext().execute_with(|| {
		...
		// Authorize and seed `bytes_permanent` directly (simulates a past renew).
		assert_ok!(TransactionStorage::authorize_account(RuntimeOrigin::root(), who, 0, 4000));
...
/// `Transactions` are the source of truth for renewed bytes.
#[test]
...
/// existing entry has `kind == Renew`, so the per-account check and increment are skipped.
#[test]
fn store_and_renew_at_exact_allowance_then_re_renew_succeeds() {
	new_test_ext().execute_with(|| {
		...
		// Re-renew same content: succeeds because the entry at block 3 has kind=Renew
		// (it was created by the first renew), so it's a re-renewal — per-account
		// bytes_permanent check and increment are skipped.
		run_to_block(5, || None);
		...
		// bytes_permanent still 2000 (re-renewal skipped the increment).
...
/// `PermanentStorageUsed` (chain-wide) decrements when renewed data ages out.
/// `bytes_permanent` reports 0 because the authorization itself has expired by then
/// (`authorization_extent` returns default for expired authorizations).
#[test]
fn expired_renewed_entry_frees_chain_cap_but_not_per_account() {
	new_test_ext().execute_with(|| {
		run_to_block(1, || None);
		let who = 1;
		let data = vec![42u8; 2000];
		...
		// Authorization expired (created at block 1, period=10, expired at block 11).
		// `authorization_extent` returns default (all zeros) for expired authorizations.
		assert_eq!(
			TransactionStorage::account_authorization_extent(who).bytes_permanent,
			0,
			"bytes_permanent reports 0 because the authorization is expired",
		);
	});
}

/// `bytes_permanent` is bounded by `bytes_allowance` across distinct content items.
/// With `bytes_allowance = 4000`, two different 2000-byte items fill the cap exactly.
/// A third distinct item is rejected.
#[test]
fn distinct_content_renewals_bounded_by_allowance() {
	new_test_ext().execute_with(|| {
		run_to_block(1, || None);
		let who = 1;
		...
		// Renew A: bytes_permanent = 0 → 2000.
		run_to_block(3, || None);
		...
		// Renew B: bytes_permanent = 2000 → 4000 (at cap).
		run_to_block(4, || None);
		...
		// Renew C: blocked (bytes_permanent=4000, 4000 + 2000 > 4000).
		run_to_block(5, || None);
...
/// counters — including `bytes_permanent` — reset to zero, enabling fresh renew cycles.
#[test]
fn re_authorization_after_expiry_resets_permanent_counter() {
	new_test_ext().execute_with(|| {
		...
		// Renew A: bytes_permanent saturates the allowance.
		run_to_block(3, || None);
		let renew_a = Call::renew { block: 1, index: 0 };
		assert_ok!(TransactionStorage::pre_dispatch_signed(&who, &renew_a));
		assert_ok!(Into::<RuntimeCall>::into(renew_a).dispatch(RuntimeOrigin::none()));
		...
		// Renew B: blocked (bytes_permanent=2000, 2000 + 2000 > 2000).
		run_to_block(5, || None);
		let renew_b = Call::renew { block: 1, index: 1 };
		assert_noop!(
		...
		// Re-authorize: all consumed counters reset to 0.
		assert_ok!(TransactionStorage::authorize_account(RuntimeOrigin::root(), who, 0, 2000));
		assert_eq!(
			TransactionStorage::account_authorization_extent(who),
			AuthorizationExtent {
...
// ---- AccountRenewals per-(account, content_hash) tracking tests ----

/// Two different accounts renewing the same content must each be charged `bytes_permanent`.
/// Under the old per-entry `kind` check, Account2 would have skipped the charge because the
/// entry already had `kind == Renew` from Account1's renewal.
#[test]
fn different_account_renew_charges_both() {
	new_test_ext().execute_with(|| {
		run_to_block(1, || None);
		let who1 = 1;
		let who2 = 2;
		...
		// Authorize both accounts with enough allowance for one renewal each.
		assert_ok!(TransactionStorage::authorize_account(RuntimeOrigin::root(), who1, 0, 2000));
		assert_ok!(TransactionStorage::authorize_account(RuntimeOrigin::root(), who2, 0, 2000));

		// Account1 stores data.
		let store_call = Call::store { data };
		assert_ok!(TransactionStorage::pre_dispatch_signed(&who1, &store_call));
		assert_ok!(Into::<RuntimeCall>::into(store_call).dispatch(RuntimeOrigin::none()));

		// Account1 renews the stored data.
		run_to_block(3, || None);
		let renew_call = Call::renew { block: 1, index: 0 };
		assert_ok!(TransactionStorage::pre_dispatch_signed(&who1, &renew_call));
		assert_ok!(Into::<RuntimeCall>::into(renew_call).dispatch(RuntimeOrigin::none()));

		assert_eq!(
			TransactionStorage::account_authorization_extent(who1).bytes_permanent,
			2000,
			"Account1 must be charged bytes_permanent on first renew",
		);

		// Account2 renews the same content (now at block 3, kind=Renew from Account1's
		// renewal). Under the new AccountRenewals tracking, Account2 has NOT renewed this
		// content before, so it must be charged.
		run_to_block(5, || None);
		let renew_call2 = Call::renew { block: 3, index: 0 };
		assert_ok!(TransactionStorage::pre_dispatch_signed(&who2, &renew_call2));
		assert_ok!(Into::<RuntimeCall>::into(renew_call2).dispatch(RuntimeOrigin::none()));

		assert_eq!(
			TransactionStorage::account_authorization_extent(who2).bytes_permanent,
			2000,
			"Account2 must also be charged bytes_permanent for the same content",
		);

		// Both accounts should have AccountRenewals entries.
		let content_hash = blake2_256(&vec![42u8; 2000]);
		assert!(AccountRenewals::contains_key(who1, content_hash));
		assert!(AccountRenewals::contains_key(who2, content_hash));
	});
}

/// Same account re-renewing the same content is free (bytes_permanent not incremented).
#[test]
fn same_account_re_renew_is_free() {
	new_test_ext().execute_with(|| {
		run_to_block(1, || None);
		let who = 1;
		let data = vec![42u8; 2000];
		...
		// Re-renew same content: free (bytes_permanent stays at 2000).
		run_to_block(5, || None);
		let renew_call2 = Call::renew { block: 3, index: 0 };
		assert_ok!(TransactionStorage::pre_dispatch_signed(&who, &renew_call2));
		assert_ok!(Into::<RuntimeCall>::into(renew_call2).dispatch(RuntimeOrigin::none()));
		assert_eq!(
			TransactionStorage::account_authorization_extent(who).bytes_permanent,
			2000,
			"re-renewal of same content by same account must not increment bytes_permanent",
		);

		// Chain-wide counter still increments (each physical entry uses space).
		// First renew = 2000, re-renew = 2000 → total 4000.
		assert_eq!(PermanentStorageUsed::get(), 4000);
	});
}

/// When an authorization expires and is re-granted, `AccountRenewals` is cleared so the
/// account is charged again for the same content in the new window.
#[test]
fn account_renewals_cleared_on_re_authorization() {
	new_test_ext().execute_with(|| {
		run_to_block(1, || None);
		let who = 1;
		let data = vec![42u8; 2000];
		...
		// First renew: charged; AccountRenewals entry inserted.
		run_to_block(3, || None);
		let renew_call = Call::renew { block: 1, index: 0 };
		assert_ok!(TransactionStorage::pre_dispatch_signed(&who, &renew_call));
		assert_ok!(Into::<RuntimeCall>::into(renew_call).dispatch(RuntimeOrigin::none()));
		assert!(AccountRenewals::contains_key(who, content_hash));
		assert_eq!(TransactionStorage::account_authorization_extent(who).bytes_permanent, 2000);

		// Force expiry and re-authorize.
		Authorizations::mutate(AuthorizationScope::Account(who), |maybe_auth| {
			let auth = maybe_auth.as_mut().expect("authorization present");
			auth.expiration = 3; // already at block 3 → expired
		});
		assert_ok!(TransactionStorage::authorize_account(RuntimeOrigin::root(), who, 0, 2000));

		// AccountRenewals must have been cleared.
		assert!(
			!AccountRenewals::contains_key(who, content_hash),
			"AccountRenewals must be cleared on re-authorization after expiry",
		);

		// Store same data again and renew — must be charged (fresh window).
		let store_call2 = Call::store { data };
		assert_ok!(TransactionStorage::pre_dispatch_signed(&who, &store_call2));
		assert_ok!(Into::<RuntimeCall>::into(store_call2).dispatch(RuntimeOrigin::none()));
		...
		assert_eq!(
			TransactionStorage::account_authorization_extent(who).bytes_permanent,
			2000,
			"fresh window must charge bytes_permanent again for same content",
		);
	});
}

/// After `remove_expired_account_authorization`, `AccountRenewals` entries for that
/// account must be cleared.
#[test]
fn account_renewals_cleared_on_remove_expired() {
	new_test_ext().execute_with(|| {
		run_to_block(1, || None);
		let who = 1;
		let data = vec![42u8; 2000];
		...
		// Remove the expired authorization.
		assert_ok!(TransactionStorage::remove_expired_account_authorization(
		...
		assert!(
			!AccountRenewals::contains_key(who, content_hash),
			"AccountRenewals must be cleared when expired authorization is removed",
		);
	});
}
