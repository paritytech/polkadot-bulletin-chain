...
/// for store, `extent.bytes += size`; for renew, `extent.bytes_permanent += size`;
/// in either case `extent.transactions_used += 1`. The boost decision then reduces
/// to a pure check of "would this leave the holder in-budget on both axes?".
pub trait BoostStrategy: Clone + PartialEq + Eq {
	fn boost(extent: AuthorizationExtent) -> TransactionPriority;
}

/// Returns whether `extent` (already post-this-tx) is in-budget on both the
/// combined byte counter and the transaction counter. The `bytes_allowance == 0`
/// guard catches the "missing or empty grant" case.
fn in_budget(extent: &AuthorizationExtent) -> bool {
	if extent.bytes_allowance == 0 {
		return false;
	}
	let combined = extent.bytes.saturating_add(extent.bytes_permanent);
	combined <= extent.bytes_allowance
...
impl BoostStrategy for ProportionalBoost {
	fn boost(extent: AuthorizationExtent) -> TransactionPriority {
		...
		let combined = extent.bytes.saturating_add(extent.bytes_permanent);
		let bytes_rem = extent.bytes_allowance.saturating_sub(combined);
		let bytes_share = (ALLOWANCE_PRIORITY_BOOST as u128 * bytes_rem as u128)
			/ extent.bytes_allowance as u128;
	...
	<T::RuntimeOrigin as OriginTrait>::PalletsOrigin: Clone + TryInto<Origin<T>>,
{
	...
	fn validate(
		...
		// Both `store{,_with_cid_config}` and `renew` consume the holder's per-period
		// budget and compete for blockspace, so both go through the same boost decision.
		// In-budget calls (store under cap, or renew — which is hard-rejected past the cap
		// before we get here) land at `base + ALLOWANCE_PRIORITY_BOOST`; over-budget
		// stores fall to `base`.
		let (this_tx_bytes, is_renew) = match inner_call {
			Call::store { data } | Call::store_with_cid_config { data, .. } =>
				(data.len() as u64, false),
			Call::renew { block, index } => match Pallet::<T>::transaction_info(*block, *index) {
				Some(info) => (info.size as u64, true),
				// If we can't resolve the renewed item here, decline to boost; the
				// later check_authorization path will reject the call regardless.
		...
		// `Origin::Authorized`; only the account-scoped variant consumes the caller's
		// allowance. Boost against the post-this-tx state so a single oversized tx is
		// demoted on entry.
		let priority = match origin.caller().clone().try_into() {
			Ok(Origin::<T>::Authorized { who, scope: AuthorizationScope::Account(_) }) => {
				...
				if is_renew {
					extent.bytes_permanent = extent.bytes_permanent.saturating_add(this_tx_bytes);
				} else {
...
mod boost_tests {
	...
	/// Build a post-this-tx extent with `bytes` consumed against `allowance`. Tx counter
	/// is parked at `(0, u32::MAX)` so the tx axis is never the binding constraint in
	/// these byte-focused tests; tx-axis behavior is covered by the dedicated test below.
	...
	fn tx_axis_gates_boost_independently() {
		// In-budget on bytes, over on transactions → no boost (FlatBoost), zero (Proportional).
