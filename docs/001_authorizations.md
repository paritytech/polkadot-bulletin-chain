...
## `on_initialize` weight audit

The obsolete-block cleanup in `on_initialize` is bounded by the
`pallet_bulletin_transaction_storage::on_initialize_with_expiry` benchmark.
Per expiring tx: 2 reads (`TransactionByContentHash::get`,
`AutoRenewals::get`) + 1 write (`TransactionByContentHash::remove`), plus a
constant 5 reads + 3 writes for the sweep itself.

At `n = MaxBlockTransactions = 512` the measured worst case is
**1029 reads + 515 writes** (constant + linear). Combined with the
`apply_block_inherents(512)` mandatory inherent that runs in the same block,
the **mandatory floor is ~207 ms ref_time and ~2.78 MiB proof_size** —
~10% of `max_block.ref_time` (2 s) and ~27% of `max_block.proof_size`
(10 MiB). Plenty of headroom for user `store` / `renew` traffic in
`DispatchClass::Normal`.

The fit is enforced by the
[`transaction_storage_weight_sanity`](../runtimes/bulletin-westend/tests/tests.rs)
test in each runtime, which calls
`pallet_bulletin_transaction_storage::ensure_weight_sanity` and asserts that
`on_initialize_with_expiry(MaxBlockTransactions) +
apply_block_inherents(MaxBlockTransactions) ≤ max_block`. Any future change
that pushes either weight up will fail the test before reaching CI.
