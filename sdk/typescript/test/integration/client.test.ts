...
describe("AsyncBulletinClient Integration Tests", { timeout: 120_000 }, () => {
  ...
  beforeAll(async () => {
    ...
    //
    // CI defaults to `in_block` so chunked flows don't wait on finalization
    // between chunks. Tests that specifically need finalization can still
    // request it via `.withWaitFor("finalized")`.
    client = new AsyncBulletinClient(api, signer, papiClient.submit, {
      txTimeout: 120_000,
      defaultWaitFor: "in_block",
    })
  ...
  describe("Store Operations", () => {
    ...
    it(
      "should fire progress events in correct order during chunked upload",
      {
        timeout: 180_000,
      },
      async () => {
    ...
    it(
      "should fire chunk events sequentially (each chunk submitted before next starts)",
      {
        timeout: 180_000,
      },
      async () => {
    ...
    it(
      "should include CID in chunk_completed events",
      {
        timeout: 180_000,
      },
      async () => {
    ...
    it(
      "should fire chunk_completed via store() builder for large data",
      {
        timeout: 180_000,
      },
      async () => {
