...
def label_for(payload: dict, fallback: str, *, size_only: bool = False) -> str:
...
def pick_label_mode(payloads: list[dict]) -> bool:
    """Return True if labels should show payload size only.

    Heuristic: a payload sweep is when account counts are constant but
    payload sizes vary across the inputs.
    """
    if len(payloads) < 2:
        return False
    accts = {p.get("config", {}).get("accounts", 1) for p in payloads}
    sizes = {p.get("config", {}).get("payloadSize") for p in payloads}
    return len(accts) == 1 and len(sizes) == len(payloads)
...
def main() -> int:
    ...
    size_only = pick_label_mode(payloads)
    labels = [
        label_for(p, args.inputs[i].stem, size_only=size_only)
    ...
    p90 = np.array([float((r.get(key) or {}).get("p90", 0.0)) for r in results])
    p99 = np.array([float((r.get(key) or {}).get("p99", 0.0)) for r in results])
    # Convert ms → s for plotting
    p50, p90, p99 = p50 / 1000, p90 / 1000, p99 / 1000
    ...
    ax_right.set_title(f"{side_label} Latency (p50 / p90 / p99)")
